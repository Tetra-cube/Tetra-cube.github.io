...
var Content =
{
	...
	{
		let chaObj = {}, age = Random.Num(item.maxage - item.minage) + item.minage;
		age += (age == "1" ? " year" : " years");			// Extremely rare edge case but it can happen
		chaObj.Age = age;
		
		let heightmod = Random.DiceRoll(item.heightmod), intHeight = item.baseheight + heightmod;
...
var HTMLStrings = 
{
	Get: function(item)
	{
		let itemContent = item.content, stringBuffer = [];
...
var Life =
{
	...
	Siblings: function(parents)	// Determine who our siblings are
	{
		...
		for(let sibNum = 0; sibNum < numSiblings; sibNum++)
		{
			...
			let birthOrderRoll = Random.DiceRoll("2d6"), birthOrder;
	...
	{
		let siblingRace = sibling.Race, name;
